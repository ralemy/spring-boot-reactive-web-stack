...
public abstract class Assertions<T extends Assertions<T>> {
    ...
    static AnnotationAssertions getAnnotation(Annotation[] annotations, String parentName, String name , boolean not) {
        Optional<Annotation> match = Arrays
                .stream(annotations)
                .filter(a -> a.annotationType().getName().contains(name))
                .findAny();
        if (!match.isPresent() && !not)
            Assert.fail(parentName + " has no annotation by name of " + name);
        else if(match.isPresent() && not)
            Assert.fail(parentName + " has an annotation by name of " + name);
        else if(!match.isPresent() && not)
            return new AnnotationAssertions(name);
    ...
    public T assertType(Type actual, TypeDef expected, String containerName){
        boolean result =  compareType(actual,expected,containerName);
        if(not)
            Assert.assertFalse(containerName +
                    " is of type " + actual.getTypeName()
                    , result);
        else
            Assert.assertTrue(containerName +
                    " is of type " + actual.getTypeName() +
