...
public class FieldAssertions extends Assertions<FieldAssertions> {
    ...
    public AnnotationAssertions Annotation(String name) {
        return getAnnotation(field.getAnnotations(), field.getName(), name, not);
    }
    ...
    public FieldAssertions isOfType(TypeDef p) {
        boolean result = compareType(field.getGenericType(), p, field.getName());
        String msg = field.getName() + " is of type " + field.getGenericType().getTypeName();
        if (not)
            Assert.assertFalse(msg, result);
        else
            Assert.assertTrue(msg + " expected " + p.typeName(), result);
    ...
    public boolean isSubtypeOf(Class<?> ref) {
        return ref.isAssignableFrom(field.getType());
    }
    ...
    public FieldAssertions isOfType(String ref) throws Throwable {
        return isOfType(Class.forName(ref));
    }
    ...
    public FieldAssertions isMockedIn(Object instance) throws IllegalAccessException {
        Object value = getValue(instance);
        if (value == null)
            fail(field.getName() + " is null");
        if (not)
            assertFalse(field.getName() + " is mocked", MockUtil.isMock(value));
        else
            assertTrue(field.getName() + " is not mocked", MockUtil.isMock(value));
        return chain();
    }
}
